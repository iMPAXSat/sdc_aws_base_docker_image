...
### **ECR Repos:** iMPAX Docker Lambda Base Images

- Development: `public.ecr.aws/w5r9l1c8/dev-impax-swsoc-docker-lambda-base`
- Production: `public.ecr.aws/w5r9l1c8/impax-swsoc-docker-lambda-base`
...
building or publishing. Each successful publish creates a unique immutable tag
plus the environment's convenience `latest` tag.

The sorting, processing, and artifacts builds are then started from their exact
`main` source. They receive `CDK_ENVIRONMENT` and the immutable
`PUBLIC_ECR_REPO` URI in one CodeBuild override list, ensuring every component
uses the base image from the same build. Release tags target production;
untagged `main` builds target development unless production is explicit.
...
The `concating` Lambda is not fanned out because the iMPAX Terraform
configuration does not currently enable that component.
